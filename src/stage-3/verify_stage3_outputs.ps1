...
$required = @(
    "topic07_fair_comparison.csv",
    "topic07_fair_comparison.json"
)
...
Write-Host ""
Write-Host "Stage 3 output verification" -ForegroundColor Cyan
...
if ($missing.Count -eq 0 -and $empty.Count -eq 0) {
    Write-Host ""
    Write-Host "PASS: all required Stage 3 output files exist and are non-empty." -ForegroundColor Green
    exit 0
}
