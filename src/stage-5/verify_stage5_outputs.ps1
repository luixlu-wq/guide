...
$required = @(
    "raw_outputs_before.jsonl",
    "raw_outputs_after.jsonl",
    "metrics_before.csv",
    "metrics_after.csv",
    "format_validity_report.json",
    "hallucination_audit.md",
    "final_prompt_selection.md",
    "reproducibility.md"
)
...
Write-Host ""
Write-Host "Stage 5 output verification" -ForegroundColor Cyan
...
if ($missing.Count -eq 0 -and $empty.Count -eq 0) {
    Write-Host ""
    Write-Host "PASS: all required Stage 5 output files exist and are non-empty." -ForegroundColor Green
    exit 0
}
